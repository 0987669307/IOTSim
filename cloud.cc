...
double distance(Ptr<Node> node1, Ptr<Node> node2){
  double x1=posX(node1);
  double y1=posY(node1);
  double x2=posX(node2);
  double y2=posY(node2);
...
void isStop(){
  q++;
std::cout<<"goi ham isStop: "<<q<<std::endl;
int s1=1,s2=1;
...
for(int i=0;i<numUav;i++){
  ...
  if(s1==1&&s2==1){
    double totalEnergy=energyInit*(numUav-1);
    double totalTime=0;
    ...
    std::cout<<"Tong data: "<<totalData/1024.0<<" (MB)"<<std::endl;
...
void sendTask(Ptr<Socket> socket,int packetSize,int idTask){
  ...
  socket->Send(packet);
  eng[id]-=0.05*packetSize;
  changeCoeff(id, 50);
  //std::cout<<"Node "<<id<<" send task at: "<<Simulator::Now().GetSeconds()<<std::endl;
}
...
void Receive(Ptr<Socket> socket){
  ...
  eng[idNode]-=0.01*(packet->GetSize());
  packet->CopyData(buffer,packet->GetSize());
  std::string str=std::string((char*)buffer);
    int id=atoi(str.substr(0,2).c_str());
    std::string s=str.substr(2,14);
  if(s=="Done")
  {      
...
void CalculateCost(Ptr<Node> node,int idTask){
  int iduav=node->GetId();
  std::cout<<"At "<<Simulator::Now().GetSeconds()<<", node "<<iduav<<" need help, idTask: "<<idTask<<std::endl;

  int index=numUav-1;

  Ptr<Socket> src=Socket::CreateSocket(uav.Get(iduav),tid);
    InetSocketAddress address = InetSocketAddress(wifiInterfaces.GetAddress(index,0), 80);
      src->Connect(address);
  sendTask(src,dataSend,idTask); 

    Ptr<Socket> s=Socket::CreateSocket(uav.Get(index),tid);
    InetSocketAddress addr = InetSocketAddress(wifiInterfaces.GetAddress(iduav,0), 80);
      s->Connect(addr);
       Simulator::Schedule(Seconds(responseTime),&sendResultTask,s,dataResult);
...
void doTask(Ptr<Node> node1, int z){
  ...
  if(stateoftask[i]==0){ 
    ...
    if(k==2||k==5||k==8)
    {
      Simulator::Schedule(Seconds(dist/vuav),& CalculateCost,node1, i);
      Simulator::Schedule(Seconds(responseTime+dist/vuav),&taskComplete,node1,z,i);
      break;
    }
    else
    {
