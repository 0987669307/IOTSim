...
void ConnectRequest (Ptr<Socket> socket, uint32_t packetSize){
...
if(isconnect[clientId]==0){ 
    std::ostringstream message;
     if(clientId < 10)
      ...
      message << clientId;
    int cleanSession=rand()%4;
    message<<cleanSession;
    int keepalive=888;
        if(keepalive < 10)
      message << "00" << keepalive;
    ...
    socket->Send (packet);
   std::cout <<"Client  send connect message: "<<message.str()<<" at "<<Simulator::Now().GetSeconds()<<std::endl;
   }   
}
void ClientReceive(Ptr<Socket> socket){
 Ptr<Packet> packet = socket->Recv(); 
  ...
  std::string str = std::string((char*)buffer);
   int returncode = atoi(str.substr(1,2).c_str());
   int clientId=socket->GetNode()->GetId();
   if(returncode==0){
    std::cout<<"Client "<<clientId<<" receive: "<<str<<" ( connection ACCEPTED ) at "<< Simulator::Now().GetSeconds()<<std::endl;
isconnect[clientId]=1;
numconnection++;
if(numconnection==numSensors){
  Simulator::Stop();
  }
}
else
      std::cout<<"Client  receive: "<<str<<" ( connection REFUSED ) at "<< Simulator::Now().GetSeconds()<<std::endl;
}
void ConnackResponse(Ptr<Socket> socket,Ptr<Packet> packet){
socket->Send(packet);
}
void BrokerReceive (Ptr<Socket> socket){
  ...
  int cleanSession = atoi(str.substr(3,1).c_str());
  int keepalive = atoi(str.substr(4,3).c_str());
  std::cout<<"Broker received connect message from client: "<<str<<" at "<< Simulator::Now().GetSeconds()<<std::endl;
std::ostringstream msg;
int sessionPresent=0;
msg<<sessionPresent;
int returncode;
if(cleanSession<2&&keepalive<1000)
  returncode=0;
else
  returncode=1;
msg<<returncode;
...
ConnackResponse(sourcebroker,packet1);
  std::cout<<"Broker send connack to client "<<clientId<<" at "<< Simulator::Now().GetSeconds()<<std::endl;
...
InetSocketAddress anyAddress = InetSocketAddress (Ipv4Address::GetAny (), 1883);
 ...
 for(int k=0;k<10;k++){
   for(int i = 0; i < numSensors; i++){
      Simulator::Schedule (Seconds ((double)10*k), &ConnectRequest, 
