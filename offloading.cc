...
#define vuav 5.0
#define numTask 3
#define dataSend 5000
#define dataResult 10000
using namespace ns3;
double t1=0,t2=0;
NodeContainer c;
int packetSize=500;
...
double energy[numNode]={99999,20000,20000,20000,20000};
int responseTime[numNode][numTask];
int data[numNode][numTask];
int timeToDo[numEvent];
int state[numNode];
int trafficdata[numNode][numNode];
...
void IsStop(){
	...
	if(s==1){
		...
		std::cout<<"Trafic matrix:"<<std::endl;
		for(int i=0;i<numNode;i++){
			for(int j=0;j<numNode;j++){
				std::cout<<trafficdata[i][j]<<"\t";
			}
			std::cout<<std::endl;
		}
  Simulator::Stop();
...
//	std::cout<<z1.size()<<" "<<z2.size()<<" "<<z3.size()<<" "<<z4.size()<<std::endl;
	IsStop();
}
void sendTask(Ptr<Socket> socket,int packetSize,int idTask){
...
void sendResultTask(Ptr<Socket> socket, int packetSize){
	std::ostringstream msg;
	int id=socket->GetNode()->GetId();
	msg<<id;
	msg<<"Done";
	Ptr<Packet> packet=Create<Packet>((uint8_t*)msg.str().c_str(),packetSize);
	socket->Send(packet);
	std::cout<<"Node "<<id<<" send result at "<<Simulator::Now().GetSeconds()<<std::endl;
}
void CalculateCost(Ptr<Node> node,int id){
	int idTask;
	if(id==14||id==15) idTask=0;
	if(id==16||id==17) idTask=1;

	if(id==18||id==19) idTask=2;

	updateEnergy();
	double cost[numNode];
	std::cout<<"At "<<Simulator::Now().GetSeconds()<<",node "<<node->GetId()<<" need help, idTask: "<<id<<":"<<std::endl;
	for(int i=0;i<numNode;i++){
		cost[i]=0.01*distance(x[idTask],y[idTask],c.Get(i))+5*responseTime[i][idTask]+0.1*data[i][idTask]+(double)4500000.0/energy[i];
		std::cout<<"Cost["<<i<<"]="<<cost[i]<<std::endl;
	}
	int index=timIdMin(cost,numNode);
	std::cout<<"Choose node "<<index<<std::endl;
	Ptr<Socket> src=Socket::CreateSocket(c.Get(node->GetId()),tid);
		InetSocketAddress address = InetSocketAddress(wifiInterfaces.GetAddress(index,0), 80);
  		src->Connect(address);
  		sendTask(src,dataSend,id);
  		trafficdata[node->GetId()][index]+=dataSend;
		capnhatheso(index,3);
 		 Simulator::Schedule(Seconds(responseTime[index][idTask]),&taskfinish,id,node->GetId());
		Ptr<Socket> s=Socket::CreateSocket(c.Get(index),tid);
		InetSocketAddress addr = InetSocketAddress(wifiInterfaces.GetAddress(node->GetId(),0), 80);
  		s->Connect(addr);
  		 Simulator::Schedule(Seconds(responseTime[index][idTask]),&sendResultTask,s,dataResult);
  		 trafficdata[index][node->GetId()]+=dataResult;
  		}
...
void Receive(Ptr<Socket> socket){
	...
	std::string str=std::string((char*)buffer);
		...
		std::string s=str.substr(1,4);
	if(str=="Done")
		std::cout<<"UAV "<<idNode<<"received result from node"<< id<<" at "<<Simulator::Now().GetSeconds()<<std::endl;
	else{
	std::string content=str.substr(1,14);
	if(content=="TaskOffLoading"){
std::cout<<"Node "<<idNode<<" received task from UAV "<<id<<" at "<<Simulator::Now().GetSeconds()<<std::endl;
	}
	else
...
/*
  ...
  for(int n=0;n<20;n++){
  for(int i=0;i<numNode;i++){
  Simulator::Schedule(Seconds(i+n*60),&advertise,c.Get(i));
  }
  }
 for(int i=14;i<numEvent;i++){
  Simulator::Schedule(Seconds(100+150*(i-14)),&CalculateCost,c.Get(zone[i]),i);
  }
  Simulator::Run();
  Simulator::Destroy();
  return 0;
...
void doTask(int zoneoftask){
	updateEnergy();
	capnhatheso(zoneoftask,2);
	switch(zoneoftask){
		case 1:{
		if(z1.size()>0&&(int)getValue(z1,0)<14){
	 ...
	 break;
		...
		case 2:{
		if(z2.size()>0&&(int)getValue(z2,0)<14){
	 ...
	 break;
		...
		case 3:{
		if(z3.size()>0&&(int)getValue(z3,0)<14){
	 ...
	 break;
		...
		case 4:{
		if(z4.size()>0&&(int)getValue(z4,0)<14){std::cout<<"UAV "<<zoneoftask<<" start task: "<<(int)getValue(z4,0)<<" at "<<Simulator::Now().GetSeconds()<< std::endl;
